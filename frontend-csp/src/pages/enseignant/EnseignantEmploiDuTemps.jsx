...
export default function EnseignantEmploiDuTemps() {
  ...
  const load = async (m = mois, y = annee) => {
    setLoading(true);
    try {
      const d = await fetchWithAuth(`/prof/emploi-du-temps?mois=${m}&annee=${y}`);
  ...
  const prev = () => {
    ...
    const mm = m === 0 ? 12 : m;
    setMois(mm); setAnnee(y); load(mm, y);
  };
  const next = () => {
    const m = mois + 1; const mm = m === 13 ? 1 : m;
    const y = m === 13 ? annee + 1 : annee;
    setMois(mm); setAnnee(y); load(mm, y);
  };
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 40, textAlign: 'center', color: T.muted }}>
          Aucune séance ce mois. Votre emploi du temps est configuré par l'administration (menu Horaires).
        </div>
      ) : (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {items.map((h) => (
                    <div key={h.id} style={{
                      ...
                      <span style={{ fontSize: 12, color: T.muted, marginLeft: 'auto' }}>{h.classe.nom}</span>
                    </div>
                  ))}
                </div>
              </div>
